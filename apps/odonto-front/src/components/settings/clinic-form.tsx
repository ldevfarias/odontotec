...
export function ClinicForm() {
    ...
    useEffect(() => {
        const fetchClinic = async () => {
            try {
                ...
                notificationService.error('Erro ao carregar dados', 'Não foi possível carregar as informações da clínica.');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleLogoChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file.size > MAX_SIZE_BYTES) {
            notificationService.error('Arquivo muito grande', `O tamanho máximo permitido é ${MAX_SIZE_MB}MB.`);
            event.target.value = '';
            return;
        }
