...
import { ANAMNESIS_TEMPLATE, QuestionType, AnamnesisCategory, AnamnesisQuestion } from '@/constants/anamnesis-template';
...
export function AnamnesisForm({ initialAnswers = [], initialComplaint = '', onSubmit, isSubmitting }: AnamnesisFormProps) {
    ...
    useEffect(() => {
        ...
        const merged = ANAMNESIS_TEMPLATE.map(q => {
            ...
            return {
                questionId: q.id,
                value: existing ? existing.value : (q.type === QuestionType.MULTISELECT ? [] : q.type === QuestionType.BOOLEAN ? false : ''),
    ...
    const updateAnswer = (questionId: string, value: any, details?: string) => {
        setAnswers(prev => prev.map(a =>
            a.questionId === questionId
                ? { ...a, value: value !== undefined ? value : a.value, details: details !== undefined ? details : a.details }
                : a
        ));
    };
    ...
    return (
        <div className="space-y-8">
            ...
            {categories.map(category => (
                <Card key={category} className="shadow-sm border-muted">
                    <CardHeader className="bg-muted/30 py-4">
                        <CardTitle className="text-md font-bold text-primary">{category}</CardTitle>
                    </CardHeader>
                    <CardContent className="p-6 space-y-6">
                        {ANAMNESIS_TEMPLATE.filter(q => q.category === category).map(question => {
                            ...
                            return (
                                <div key={question.id} className="space-y-3 pb-4 border-b last:border-0 last:pb-0">
                                        ...
                                        <div className="flex-shrink-0">
                                            {question.type === QuestionType.BOOLEAN && (
                                                <div className="flex items-center gap-3">
                                                    <span className={cn("text-xs font-medium", !answer.value ? "text-primary" : "text-muted-foreground")}>Não</span>
                                                    <Switch
                                                        checked={answer.value}
                                                        onCheckedChange={(val) => updateAnswer(question.id, val)}
                                                    />
                                                    <span className={cn("text-xs font-medium", answer.value ? "text-primary" : "text-muted-foreground")}>Sim</span>
                                                </div>
                                            )}

                                            {question.type === QuestionType.SELECT && (
                                                <Select value={answer.value} onValueChange={(val) => updateAnswer(question.id, val)}>
                                    ...
                                    {question.type === QuestionType.MULTISELECT && (
                                        <div className="flex flex-wrap gap-2">
                                            {question.options?.map(opt => {
                                                const isSelected = Array.isArray(answer.value) && answer.value.includes(opt);
                                                return (
                                                    <Badge
                                                        key={opt}
                                    ...
                                    {(question.type === QuestionType.TEXT || answer.value === true || (Array.isArray(answer.value) && answer.value.length > 0) || (typeof answer.value === 'string' && answer.value !== 'Não' && answer.value !== '')) && (
                                        <Textarea
