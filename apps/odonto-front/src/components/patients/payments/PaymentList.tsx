/* eslint-disable max-lines -- dense UI table/accordion component extracted from PaymentsTab.tsx */
'use client';
...
import { CreatePaymentDtoMethodEnumKey } from '@/generated/ts/CreatePaymentDto';
...
export type EnrichedPlan = TreatmentPlan & { totalPaid: number; balance: number };
...
interface PaymentListProps {
  ...
  onOpenPaymentDialog: (planId: number, balance: number) => void;
  onStatusUpdate: (id: number, status: UpdateTreatmentPlanDtoStatusEnumKey) => void;
  onRegisterStandalone: () => void;
}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {isApproved && plan.balance > 0 && (
                            <Button
                              ...
                              onClick={() => onOpenPaymentDialog(plan.id, plan.balance)}
                            >
                          ...
                          {plan.status !== 'DRAFT' &&
                            ...
                            plan.status !== 'COMPLETED' && (
                              <Button
                                ...
                                onClick={() => onStatusUpdate(plan.id, 'CANCELLED')}
                              >
                                Cancelar
                              </Button>
                            )}
