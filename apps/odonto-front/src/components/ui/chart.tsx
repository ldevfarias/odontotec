  ...
  return (
    <ChartContext.Provider value={{ config }}>
      <div
        ...
        ref={ref}
        className={cn(
          "[&_.recharts-cartesian-axis-tick_text]:fill-muted-foreground [&_.recharts-cartesian-grid_line[stroke='#ccc']]:stroke-border/50 [&_.recharts-curve.recharts-area-curve]:stroke-primary [&_.recharts-dot]:stroke-primary [&_.recharts-grid-line]:stroke-border/50 [&_.recharts-label]:fill-foreground [&_.recharts-rectangle.recharts-tooltip-cursor]:fill-muted [&_.recharts-reference-line_[stroke='#ccc']]:stroke-border flex aspect-video justify-center text-xs [&_.recharts-sector]:outline-none [&_.recharts-sector[stroke='#fff']]:stroke-transparent [&_.recharts-surface]:outline-none",
          className,
        )}
        {...props}
      >
    ...
    <style
      dangerouslySetInnerHTML={{
        __html: Object.entries(config)
          .map(([key, entry]) => {
            const color = entry.theme?.[Object.keys(entry.theme)[0]] || entry.color;
            return color
              ? `
[data-chart="${id}"] {
  --color-${key}: ${color};
                        ...
                        <div
                          className={cn(
                            'shrink-0 rounded-[2px] border-[--color-border] bg-[--color-bg]',
                            {
                              ...
                              'nest-dot': nestLabel && String(indicator) === 'dot',
                            },
                          )}
                          style={
                            {
