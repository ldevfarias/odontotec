...
async function verify() {
  ...
  const updated = await updateClinic(token, {
    name: 'OdontoTec Updated',
    address: 'New Address',
  });
  ...
  console.log(
    'Skipping logo upload in script (requires file handling in node fetch). Manual verification recommended.',
  );
