...
export class LocalStorageProvider implements IStorageProvider {
    ...
    async upload(file: Buffer, fileName: string, mimeType: string, folderPath: string = ''): Promise<string> {
        const uniqueFileName = `${randomUUID()}${extname(fileName)}`;
        const relativePath = folderPath ? path.join(folderPath, uniqueFileName) : uniqueFileName;
        ...
        try {
            ...
            const urlPath = folderPath ? `${folderPath}/${uniqueFileName}` : uniqueFileName;
