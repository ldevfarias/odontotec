...
const bcryptMock = require('bcrypt') as {
  compare: jest.Mock;
  hash: (typeof import('bcrypt'))['hash'];
};
...
describe('AuthService', () => {
  ...
  describe('login', () => {
    ...
    it('stores hashed refresh token after login', async () => {
      ...
      expect((updateCall as any)[1].currentHashedRefreshToken).toMatch(
        /^\$2[aby]\$/,
      );
      expect((updateCall as any)[1].currentHashedRefreshToken).not.toBe(
        mockTokens.refresh_token,
      );
    });
  ...
  describe('refreshTokens', () => {
    ...
    it('throws UnauthorizedException when user is not found', async () => {
      ...
      await expect(service.refreshTokens(99, 'any-token')).rejects.toThrow(
    ...
    it('throws UnauthorizedException when user has no stored refresh token', async () => {
      ...
      await expect(service.refreshTokens(1, 'any-token')).rejects.toThrow(
        UnauthorizedException,
      );
    });
    ...
    it('throws UnauthorizedException when refresh token does not match stored hash', async () => {
      ...
      await expect(service.refreshTokens(1, 'tampered-token')).rejects.toThrow(
        UnauthorizedException,
      );
    });
    ...
    it('throws UnauthorizedException when user is inactive', async () => {
      ...
      await expect(service.refreshTokens(1, 'valid-token')).rejects.toThrow(
        UnauthorizedException,
      );
    });
  });
  ...
  describe('verifyEmailAndSetPassword', () => {
    ...
    it('creates user and clinic and returns tokens on valid token + matching passwords', async () => {
      mockUsersService.findPendingRegistrationByToken.mockResolvedValue(
        pendingReg,
      );
      ...
      expect(mockUsersService.deletePendingRegistration).toHaveBeenCalledWith(
        pendingReg.id,
        expect.anything(),
      );
      expect(result).toMatchObject({
    ...
    it('throws BadRequestException when passwords do not match', async () => {
      ...
      expect(
        mockUsersService.findPendingRegistrationByToken,
  ...
  describe('resetPassword', () => {
    ...
    it('throws UnauthorizedException when jti does not match stored token (already used)', async () => {
      ...
      await expect(
        service.resetPassword('already-used-token', 'NewPass!'),
      ).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('throws UnauthorizedException when jwt verification fails', async () => {
      ...
      await expect(
        service.resetPassword('bad-jwt', 'NewPass!'),
      ).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('throws UnauthorizedException when token type is not reset', async () => {
      ...
      await expect(
        service.resetPassword('wrong-type-token', 'NewPass!'),
      ).rejects.toThrow(UnauthorizedException);
    });
  });
  ...
  describe('Token exposure — tokens must not appear in service return values', () => {
    ...
    it('refreshTokens() returns a message field so the controller does not respond with an empty body', async () => {
      ...
      const { _tokens, ...data } = (await service.refreshTokens(
        1,
        'valid-token',
      )) as any;
    ...
    it('registerTenant() does not return access_token or refresh_token in the top-level object', async () => {
      ...
      mockClinicsService.createForUser.mockResolvedValue({
        id: 10,
        name: 'Test Clinic',
      });
      mockUsersService.update.mockResolvedValue(undefined);
    ...
    it('registerByInvitation() does not return access_token or refresh_token in the top-level object', async () => {
      ...
      mockUsersService.completeInvitation.mockResolvedValue({
        user,
        invitation,
      });
