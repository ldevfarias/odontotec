...
export class AuthService {
  ...
  async registerByInvitation(registerDto: RegisterInvitationDto) {
    return await this.dataSource.transaction(async (manager) => {
      ...
      await this.clinicsService.addMember(
        invitation.clinicId,
        user.id,
        clinicRole,
        manager,
  ...
  async initiateRegistration(dto: InitiateRegistrationDto) {
    const pending = await this.usersService.createPendingRegistration(
      dto.name,
      dto.email,
    );
    const emailSent = await this.emailService.sendRegistrationVerificationEmail(
      pending.email,
      pending.name,
  ...
  async verifyEmailAndSetPassword(dto: VerifyEmailDto) {
    ...
    return await this.dataSource.transaction(async (manager) => {
      const pending = await this.usersService.findPendingRegistrationByToken(
        dto.token,
      );
      if (!pending) {
      ...
      const clinic = await this.clinicsService.createForUser(
        user.id,
        {
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      if (!user.termsAcceptedAt) {
        throw new BadRequestException(
          'Você precisa aceitar os termos de uso antes de configurar a clínica',
        );
      }
      ...
      // Update clinic
      await this.clinicsService.update(
        ownerClinic.clinic.id,
        {
          name: dto.clinicName,
    ...
    return await this.dataSource.transaction(async (manager) => {
      try {
        ...
        return {
          ...
          clinics: [
            { id: clinic.id, name: clinic.name, role: ClinicRole.OWNER },
          ],
          _tokens: tokens,
        };
      } catch (error) {
  ...
  async resetPassword(token: string, newPass: string): Promise<void> {
    return await this.dataSource.transaction(async (manager) => {
      try {
        ...
        const user = await this.usersService.findOneForPasswordReset(
          payload.sub,
          manager,
        );
        if (!user) throw new UnauthorizedException('User not found');
