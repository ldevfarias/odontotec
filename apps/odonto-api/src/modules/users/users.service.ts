...
export class UsersService {
  ...
  private getRepository<T extends object>(
    entityClass: new () => T,
  ...
  async findPendingRegistrationByToken(
    token: string,
  ...
  async completeInvitation(
    ...
    password: string,
    manager?: EntityManager,
  ): Promise<{ user: User; invitation: UserInvitation }> {
    return await (
      manager
        ? Promise.resolve(manager)
        : this.dataSource.transaction(async (m) => m)
    ).then(async (m) => {
  ...
  async findAllByClinic(
    ...
    limit = 50,
    role?: ClinicRole,
  ): Promise<PaginatedResponseDto<ClinicUserDto>> {
    const where: FindOptionsWhere<ClinicMembership> = {
      clinicId,
      isActive: true,
      user: Not(IsNull()),
    };
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      if (membership.avatarUrl) {
        try {
          ...
          console.warn(
            `Failed to delete old avatar from R2: ${membership.avatarUrl}`,
            err,
          );
        }
      }
