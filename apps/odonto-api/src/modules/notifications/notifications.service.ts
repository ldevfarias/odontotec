...
export class NotificationsService {
    ...
    async create(message: string, clinicId: number, type: string = 'INFO', userId?: number): Promise<Notification> {
    ...
    async markAsRead(id: number, clinicId: number): Promise<void> {
        const notification = await this.notificationsRepository.findOne({ where: { id, clinicId } });
        if (!notification) {
    ...
    async markAllAsRead(clinicId: number): Promise<void> {
        await this.notificationsRepository.update({ clinicId, read: false }, { read: true });
    }
}
