...
export class DashboardService {
  ...
  async getStats(clinicId: number): Promise<DashboardStatsDto> {
    ...
    const appointmentsResponse = await this.appointmentsService.findAll(
      clinicId,
      undefined,
      undefined,
      {
    ...
    const occupancyRate = Math.min(
      Math.round((patientsToday / capacity) * 100),
      100,
    );

    // 3. Realized Revenue (Today)
    const payments = await this.paymentsService.findByPeriod(
      startOfDay,
      endOfDay,
      clinicId,
    );
    ...
    // 4. Expected Revenue
    const treatmentPlans = await this.treatmentPlansService.findByPeriod(
      startOfDay,
      endOfDay,
      clinicId,
    );
    const expectedRevenue = treatmentPlans.reduce(
  ...
  async getRevenueHistory(
    ...
    const allPayments = await this.paymentsService.findByPeriod(
      startDate,
      rangeEnd,
      clinicId,
    );
