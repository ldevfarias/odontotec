...
export class AnamnesisService {
    ...
    async create(createAnamnesisDto: CreateAnamnesisDto, clinicId: number): Promise<Anamnesis> {
    ...
    async update(id: number, updateAnamnesisDto: UpdateAnamnesisDto, clinicId: number): Promise<any> {
    ...
    async getActiveAlerts(patientId: number, clinicId: number): Promise<{ questionId: string; label: string }[]> {
    ...
    private calculateAlerts(anamnesis: Anamnesis): { questionId: string; label: string }[] {
        ...
        for (const answer of answers) {
            const questionTemplate = ANAMNESIS_TEMPLATE.find(q => q.id === answer.questionId);
                ...
                if (isAlert) {
                    if (answer.questionId === 'allergies' && Array.isArray(answer.value)) {
                        answer.value.forEach((v: string) =>
                            alerts.push({ questionId: answer.questionId, label: `Alergia: ${v}` })
                        );
                    } else {
                        alerts.push({
