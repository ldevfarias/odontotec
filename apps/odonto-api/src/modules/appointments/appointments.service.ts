...
export class AppointmentsService {
  ...
  async updateStatusPublic(
    ...
    try {
      ...
      if (
        ...
        payload.action !== 'appointment_action'
      ) {
  ...
  async findAll(
    ...
    if (!options?.includeOccurrences) {
      query.andWhere('appointment.status NOT IN (:...badStatus)', {
        badStatus: [AppointmentStatus.CANCELLED, AppointmentStatus.ABSENT],
      });
    }
    ...
    if (role === 'DENTIST') {
      ...
      query.andWhere('appointment.dentistId = :dentistId', {
  ...
  async update(
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      const locked = await appointmentRepo
        ...
        .where('appointment.id = :id AND appointment.clinicId = :clinicId', {
          id,
          clinicId,
        })
        .getOne();
      ...
      const after = await appointmentRepo.findOne({
        where: { id, clinicId },
        relations: ['patient', 'dentist'],
      });
  ...
  private async notifyAppointmentChanges(
    before: Appointment,
    after: Appointment,
    clinicId: number,
  ): Promise<void> {
  ...
  private async notifyIfPatientChanged(
    before: Appointment,
    after: Appointment,
    clinicId: number,
  ): Promise<void> {
  ...
  private async notifyIfRescheduled(
    before: Appointment,
    after: Appointment,
    clinicId: number,
  ): Promise<void> {
    if (
      new Date(before.date).toISOString() === new Date(after.date).toISOString()
    )
      return;
  ...
  private async notifyIfDentistChanged(
    before: Appointment,
    after: Appointment,
    clinicId: number,
  ): Promise<void> {
  ...
  private async notifyIfCancelled(
    before: Appointment,
    after: Appointment,
    clinicId: number,
  ): Promise<void> {
  ...
  private async checkConflict(
    ...
    const query = repo
      .createQueryBuilder('appointment')
      .where('appointment.clinicId = :clinicId', { clinicId })
      .andWhere('appointment.status NOT IN (:...badStatus)', {
        badStatus: [AppointmentStatus.CANCELLED],
      })
      .andWhere('appointment.date < :endDate', { endDate })
      .andWhere(
        "appointment.date + (appointment.duration * interval '1 minute') > :startDate",
        { startDate },
      );
    ...
    if (dentistId && patientId) {
      query.andWhere(
        new Brackets((qb) => {
          qb.where('appointment.dentistId = :dentistId', { dentistId }).orWhere(
  ...
  async getAvailableSlots(
    ...
    const query = this.appointmentsRepository
      ...
      .andWhere('appointment.date BETWEEN :startOfDay AND :endOfDay', {
        startOfDay,
        endOfDay,
      });
