...
describe('AppointmentsService — update notifications', () => {
        ...
        getRepository: jest.fn().mockReturnValue({
          ...
          findOne: jest
            ...
            .mockImplementation((...args) =>
              (findOneSpy as jest.Mock)(...args),
            ),
        }),
        update: jest.fn().mockResolvedValue(undefined),
        findOne: jest
          .fn()
          .mockImplementation((...args) => (findOneSpy as jest.Mock)(...args)),
      };
      return cb(manager);
    }),
  };
  ...
  it('notifies dentist when patient changes', async () => {
    ...
    const after = makeAppointment({
      patientId: 99,
      patient: { id: 99, name: 'Carlos Perez' } as Patient,
    });

    findOneSpy.mockResolvedValueOnce(before).mockResolvedValueOnce(after);
  ...
  it('does NOT notify when appointment was already cancelled', async () => {
    const alreadyCancelled = makeAppointment({
      status: AppointmentStatus.CANCELLED,
    });
