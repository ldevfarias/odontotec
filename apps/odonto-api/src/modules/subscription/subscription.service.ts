...
export class SubscriptionService {
  ...
  async getStatus(user: User, clinicId: number) {
    ...
    const trialEnd =
      clinic.trialEndsAt ||
      new Date(clinic.createdAt.getTime() + 7 * 24 * 60 * 60 * 1000);
    ...
    if (clinic.stripeSubscriptionId && status !== 'EXPIRED') {
      try {
        ...
        if (syncedStatus && clinic.subscriptionStatus !== syncedStatus) {
          ...
          if ((subscription as any).current_period_end) {
            clinic.currentPeriodEnd = new Date(
        ...
        console.error(
          '[Subscription] Error fetching Stripe subscription, using cached DB status:',
          error,
        );
      }
    } else if (
      ...
      try {
        ...
        if (sub) {
          const syncedStatus =
            stripeStatusMap[sub.status] ?? clinic.subscriptionStatus;
          console.log(
        ...
        console.error(
          '[Subscription] Error in fallback subscription lookup by customer:',
          error,
        );
      }
    }
  ...
  async createCheckoutSession(
    ...
    if (!priceId) {
      throw new BadRequestException(
        'Stripe Price IDs are not configured properly',
      );
    }
    ...
    const session = await this.stripe.checkout.sessions.create({
      ...
      cancel_url:
        cancelUrl ??
        `${this.configService.get('FRONTEND_URL')}/settings/billing`,
    });
