...
describe('StripeWebhookController — private handlers', () => {
  ...
  describe('handleCheckoutSessionCompleted', () => {
    it('upgrades clinic to PRO when not yet processed', async () => {
      ...
      const mockManager = {
        getRepository: jest.fn().mockReturnValue(clinicRepository),
      };
      await (controller as any).handleCheckoutSessionCompleted(
        session,
        mockManager,
      );
    ...
    it('sends ProActivated email to owner after upgrade', async () => {
      ...
      expect(
        emailService.sendSubscriptionProActivatedEmail,
      ).toHaveBeenCalledWith('ana@example.com', 'Dr. Ana', 'Clínica Sorriso');
    });
    ...
    it('does not throw and does not save when clinic is not found', async () => {
      ...
      await expect(
        (controller as any).handleCheckoutSessionCompleted(
          session,
          mockManager,
        ),
  ...
  describe('handleInvoicePaymentSucceeded', () => {
    it('reactivates clinic and clears trial/cancelAtPeriodEnd', async () => {
      ...
      await (controller as any).handleInvoicePaymentSucceeded(
        invoice,
        mockManager,
      );
    ...
    it('does not throw and does not save when clinic is not found', async () => {
      ...
      await expect(
        (controller as any).handleInvoicePaymentSucceeded(invoice, mockManager),
      ).resolves.not.toThrow();
      expect(clinicRepository.save).not.toHaveBeenCalled();
    });
  });
  ...
  describe('handleSubscriptionUpdated', () => {
    it('sets subscriptionStatus to past_due when Stripe status is past_due', async () => {
      ...
      await (controller as any).handleSubscriptionUpdated(
        subscription,
        mockManager,
      );
    ...
    it('sends cancel-scheduled email when cancel_at_period_end transitions from false to true', async () => {
      ...
      expect(
        emailService.sendSubscriptionCancelScheduledEmail,
    ...
    it('does not throw and does not save when clinic is not found', async () => {
      ...
      await expect(
        (controller as any).handleSubscriptionUpdated(
          subscription,
          mockManager,
        ),
      ).resolves.not.toThrow();
  ...
  describe('handleSubscriptionDeleted', () => {
    it('cancels clinic: sets CANCELED, plan FREE, clears cancelAtPeriodEnd and sends email', async () => {
      ...
      await (controller as any).handleSubscriptionDeleted(
        subscription,
        mockManager,
      );
    ...
    it('does not throw and does not save when clinic is not found', async () => {
      ...
      await expect(
        (controller as any).handleSubscriptionDeleted(
          subscription,
          mockManager,
        ),
      ).resolves.not.toThrow();
