...
async function testTreatmentPlans() {
  ...
  try {
    ...
    const createPatientRes = await axios.post(
      `${baseUrl}/patients`,
      {
    ...
    const createPlanRes = await axios.post(
      `${baseUrl}/treatment-plans`,
      {
        patientId: patientId,
        dentistId: 1,
        notes: 'Extração de siso e limpeza',
        items: [
          { description: 'Extração de Siso', value: 350.5, toothNumber: 18 },
          { description: 'Limpeza Completa', value: 150.0 },
        ],
      },
      {
        headers: { Authorization: `Bearer ${token}` },
    ...
    console.log(
      `✅ Plan created with ID: ${planId}, Total: ${createPlanRes.data.totalAmount}`,
    );

    if (createPlanRes.data.totalAmount != 500.5) {
    ...
    const updateRes = await axios.patch(
      `${baseUrl}/treatment-plans/${planId}`,
      {
        status: 'APPROVED',
      },
      {
    ...
    // 5. Verify Tenancy
    console.log(
      '5. Verifying Tenancy (attempting to access from non-existent clinic)...',
    );
